/*
    Модифицировано hexaedron для отсчёта UNIX-времени с 01.01.2000 
    для совместимости с либами для Arduino
...
struct SunPosition {
    ...
    SunPosition(float lat, float lon, uint32_t unix2000, int16_t gmt = 0) {
        compute(lat, lon, unix2000, gmt);
    }

    // выполнить расчёт, указать широту (градусы), долготу (градусы), unix время (секунды), часовой пояс в часах или минутах
    void compute(float lat, float lon, uint32_t unix2000, int16_t gmt = 0) {
        lat = radians(lat);
        float hours = (unix2000 % 86400ul) / 86400.0;       // decimal hours
#ifndef SP_PRECISE
        int day = fmod(unix2000 / 86400.0, 365.25);
...
#else
        float JulCent = (unix2000 / 86400.0 - .5) / 36525.0;                                                   // Julian Century
